...
# DeepseekV3 Mock Implementation (from first script)
...
class DeepseekV3ForCausalLM(PreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        # Calculate loss if labels are provided (needed for training)
        loss = None
        ...
        # Return object with loss attribute (compatible with training script)
        class ModelOutput:
...
def get_model_and_dataloader(model_name, dataset_name, hidden_size):
    ...
    elif model_name == "kimi_k2":
        tokenizer = AutoTokenizer.from_pretrained(
            "moonshotai/Moonlight-16B-A3B-Instruct", trust_remote_code=True
        ) ## moonshotai/Moonlight-16B-A3B-Instruct # ## moonshotai/Moonlight-16B-A3B-Instruct 
    else:
...
def get_optimizer(optimizer_name, model, lr=1e-3, wd=0.1):
    if optimizer_name == "adamw":
        ...
        muon_params = [
            ...
            if p.ndim >= 2 and "embed_tokens" not in name and "lm_head" not in name and "word_embeddings" not in name
        ]
        adamw_params = [
            p
            for name, p in model.named_parameters()
            if not (
                p.ndim >= 2 and "embed_tokens" not in name and "lm_head" not in name and "word_embeddings" not in name
            )
        ]
...
# Additional utility functions from first script
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="kimi_k2")
    ...
    # Print model statistics for kimi_k2
    if args.model == "kimi_k2":
        total_params = count_total_parameters(model)
        active_params = count_active_parameters(model.config, total_params)
        logger.info(f"Total parameters: {total_params:,}")
        logger.info(f"Active parameters (approx.): {active_params:,}")
